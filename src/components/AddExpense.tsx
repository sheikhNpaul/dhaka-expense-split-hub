...
export const AddExpense = ({ onClose, onExpenseAdded, currentHomeId, defaultDate }: AddExpenseProps) => {
  ...
  const fetchHomeMembers = useCallback(async () => {
    try {
      // First get home members
      const { data: membersData, error: membersError } = await supabase
        .from('home_members')
        .select('id, user_id, profile:profiles(id, name, email)')
  ...
  useEffect(() => {
    if (currentHomeId && user) {
      fetchCategories();
      fetchHomeMembers();
    }
  }, [currentHomeId, user, fetchCategories, fetchHomeMembers]);
